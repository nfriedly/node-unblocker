"use strict";
const debug = require("debug")("unblocker:charsets");
...
function charsets(config) {
  function decodeCharset(data) {
    if (contentTypes.shouldProcess(config, data)) {
      ...
      // fixup html (but not js)
      if (mayContainMeta(data.contentType)) {
...
const re_charset_finder = /<\?xml[^>]+encoding="([^">]+)"|<meta [^>]*charset=['"]?([^ '">]+)['"]/i; // warning: making this global causes it to not include the matched value in the results :/
...
IconvHtmlStream.prototype.bufferAndTest = function (chunk, encoding, done) {
  ...
  const str = this.buff.toString();
  const charsetMatch = str.match(re_charset_finder); // extract the charset from a meta tag or the opening <?xml tag
  const endOfHead = str.match(/<\/head>/); // todo: consider matching on some other tags such as |<div |<span <a | to avoid buffering entire html snippets
  if (charsetMatch) {
...
// todo: make this part of the parser
const re_charset_replacer = /<\?xml[^>]+encoding="([^">]+)"|<meta [^>]*charset=['"]?([^ '">]+)['"]/gi; // similar to the charset_finder, except global
    ...
    chunk
      ...
      .replace(
        ...
        function (subChunk, xmlCharset, metaCharset) {
          const oldCharset = xmlCharset || metaCharset;
          const newSubChunk = subChunk.replace(oldCharset, "UTF-8");
          debug(
