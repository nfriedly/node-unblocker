...
function charsets(config) {
    ...
    function decodeCharset(data, next) {
        if (contentTypes.shouldProcess(config, data)) {
            ...
            // in all cases, we output utf8, so we want to make sure any meta tags in the html match that
...
IconvHtmlStream.prototype.bufferAndTest = function(chunk, encoding, done) {
    ...
    if (charset) {
        this.startStreaming(charset, encoding, done);
    } else if (endOfHead) {
...
MetaCharsetReplacerStream.prototype._transform = function(chunk, encoding, done) {
    done(null, chunk.replace(re_charset_replacer, function(subChunk, xmlCharset, metaCharset) {
        var oldCharset = xmlCharset || metaCharset;
        var newSubChunk = subChunk.replace(oldCharset, "utf8");
