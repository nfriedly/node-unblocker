...
// todo: regex for partial capture from end of chunk
...
function rewriteChunk(chunk, state = {}) {
  ...
  // done searching for location matches, push any remainders
  // todo: cache remainder or maybe cache it for the next pass
  if (loc === -1) {
    output.push(chunk);
  } else {
    output.push(chunk.slice(lastLoc));
  }
