...
module.exports = function (config) {
  ...
  const RE_DUOBLE_PREFIX = new RegExp(
  ...
  function getRealUrl(path) {
    const uri = new URL(path, "http://referer.invalid");
    let real_url = uri.pathname.substr(config.prefix.length);
    real_url = real_url.replace(RE_DUOBLE_PREFIX, "$1");
    real_url = real_url.replace(RE_UNMERGE_SLASHES, "$1/$2");
    return real_url + uri.search;
  }
