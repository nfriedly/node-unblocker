...
var debug = require('debug')('unblocker');
...
function Unblocker(config) {
    ...
    config.requestMiddleware = debug(config.requestMiddleware, 'request');
    config.responseMiddleware = debug(config.responseMiddleware, 'response');


    debug("Unblocker initialized, config: %s", config);
    ...
    function thisSite(request) {
        return 'http://' + thisHost(request) + config.prefix;
    }
    ...
    function redirectTo(request, response, site, headers) {
        ...
        var location = site ? config.prefix + site : config.homepage; // todo: just call next() instead of redirecting to homepage
        try {
