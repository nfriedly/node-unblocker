...
Unblocker.urlPrefixer = require("./rewrite-html.js");
...
function Unblocker(config) {
  ...
  if (config.standardMiddleware !== false) {
    ...
    const hsts = Unblocker.hsts(config);
    const hpkp = Unblocker.hpkp(config);
    const csp = Unblocker.csp(config);
    ...
    // todo: js reqriter
    const urlPrefixer = Unblocker.urlPrefixer(config);
  ...
  const getRealUrl = Unblocker.getRealUrl(config);
  ...
  function recoverTargetUrl(request) {
    ...
    if (ref.host != thisHost(request)) {
      return null;
    }

    // now we know where they came from, so we can do something for them
    if (ref.pathname.indexOf(config.prefix + "http") === 0) {
      const real_url = getRealUrl(ref.pathname + ref.search);
      const real_uri = new URL(real_url, "http://referer.invalid");
      const target_url = real_uri.protocol + "//" + real_uri.host + request.url;
  ...
  function redirectTo(request, response, site, headers) {
    ...
    const location = request.thisSite() + site;
  ...
  function initContext(clientRequest, clientResponse, clientSocket) {
    ...
    if (url_data.pathname.indexOf(config.prefix + "http") === 0) {
      url = new URL(getRealUrl(clientRequest.url), "http://host.invalid");
    } else {
      const target = recoverTargetUrl(clientRequest);
      if (target) {
        // redirecting is difficult here, and doesn't add much value, so just use the recovered target url
        url = new URL(target, "http://host.invalid");
      }
    }

    // todo: update everything to use this
    const urlWrapper = new PrefixUrlWrapper(
      new URL(clientRequest.thisSite(), "http://proxy-host.invalid"),
      url
    );
  ...
  function handleRequest(clientRequest, clientResponse, next) {
    const data = initContext(clientRequest, clientResponse);
  ...
  handleRequest.onUpgrade = function onUpgrade(
    ...
    const data = initContext(clientRequest, null, clientSocket);
