    ...
    var remote_request = proto.request(options, function(remote_response) {
        ...
        if (needs_parsed) {
            ...
            remote_response = remote_response
                .pipe(encodingStreams.decode)
                .pipe(urlPrefixStream);

            config.responseMiddleware.forEach(function(createStream) {
                remote_response = remote_response.pipe(createStream()); // todo: pass createStream an object that makes a few things available such as the url, headers, etc.
            });
            remote_response = remote_response.pipe(encodingStreams.recode);
        }
