...
const debug = require("debug")("unblocker:url-prefixer");
...
function fixUrl(config, urlStr, base) {
  // todo: check if it's already prefixed
  // todo: check if it needs to be prefixed
...
function urlPrefixer(config) {
  ...
  const re_proxied_xmlns = new RegExp(
    '(xmlns(:[a-z]+)?=")' + config.prefix,
    "ig"
  );
  const re_proxied_doctype = new RegExp(
  ...
  const fixUrlBound = fixUrl.bind(null, config);
      ...
      data.html.on("startTag", ({ startTag }) => {
            ...
            startTag.attrs.forEach((attr) => {
              if (attr.name === "content") {
                ...
                if (position != -1) {
                  ...
                  attr.value = start + fixUrlBound(url, data.url);
                }
              }
            });
          }
        ...
        startTag.attrs.forEach((attr) => {
          if (prefixAttrs.includes(attr.name) && urlNeedsFixed(attr.value)) {
            attr.value = fixUrlBound(attr.value, data.url);
          }
          if (attr.name === "style") {
